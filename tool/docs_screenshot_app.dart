...
/// `?screen=home|queue|smart-shuffle|stream-quality|desktop-lyrics|realtime-chart|discovery|discovery-recommendations|discovery-mv|discovery-recent|discovery-new-releases|discovery-new-release-chart|live-radio|artist|artist-mv|collection-save|collection-information|hubs|top-100|release-catalog|weekly-chart|search|search-results|new-releases|player|car-mode|song-detail|lyrics|lyric-share|radio|library|for-you|analytics|wrapped|tv`.
Future<void> main() async {
  ...
  final initialQueue = screen == 'discovery-new-releases'
      ...
      : screen == 'artist' || screen == 'artist-mv'
      ? _artistDetail.songs
            .where((item) => item.playable)
            .map((item) => item.song)
            .toList(growable: false)
...
final _artistDetail = CatalogArtistDetail(
  ...
  awardCount: 12,
  featuredSongs: [
    for (final song in _artistSongs.take(2))
      CatalogSong(
        song: song,
